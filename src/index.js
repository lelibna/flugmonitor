...
async function processOverflights(flights) {
    ...
    for(const flight of flights) {
        ...
        if(isNew) {
            await processNewFlight(flight, now);
        } else {
            await processOldFlight(flight, seen, now);
        }
    }
}

async function processNewFlight(flight, now) {
    const entry = {
...
async function processOldFlight(flight, seen, now) {
    seen.time = now;
